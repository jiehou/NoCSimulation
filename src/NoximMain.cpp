...
vector<int> NoximGlobalParams::faulty_routers;
int NoximGlobalParams::target_packets = DEFAULT_TARGET_PACKETS;
bool NoximGlobalParams::mode_target_packets = false;
map<string, vector<int> >NoximGlobalParams::hops_info;
vector<string> NoximGlobalParams::processed_packets;
bool NoximGlobalParams::network_saturated = false;
int NoximGlobalParams::dropped_flits = 0;
//---------------------------------------------------------------------------
...
int sc_main(int arg_num, char *arg_vet[])
{
	...
	// inject faulty routers
	//injectFaults();
	/*
	// 17 25 27 46 51 59
	vector<int> faults = {17, 25, 27, 46, 51, 59};
	for(const auto& fault : faults)
	{
		NoximGlobalParams::faulty_routers.push_back(fault);
	}
	*/
	std::cout << "#[D] NoximGlobalParams::faulty_routers.size(): " 
			  << NoximGlobalParams::faulty_routers.size() << std::endl;
	// Signals
	...
	// @NOTE
	// When the network is saturated, the result will be INVALID
	if(NoximGlobalParams::network_saturated)
		cout << "% Network saturated: yes" << endl; 
	if(total_sim_cycles > 0)
		cout << "% Communication time: " << (total_sim_cycles - DEFAULT_RESET_TIME) << endl;
	// Show statistics
	...
	cout << "% Simulation time (ms): " << chrono::duration_cast<chrono::milliseconds>(end - start).count() << endl;
	return 0;
}