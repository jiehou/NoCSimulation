...
void showHelp(char selfname[])
{
	...
	cout << 
	"\t-packets N\tRun for the specified packets will be sent and processed (default " << DEFAULT_TARGET_PACKETS << ")" << endl;
    cout <<
	...
	else
	{
		for (int i = 1; i < arg_num; i++)
		{
			...
			else if (!strcmp(arg_vet[i], "-routing"))
			{
				...
				else if (!strcmp(routing, "ftnf"))
				{
					NoximGlobalParams::routing_algorithm = ROUTING_FTNF;
				}
			...
			else if (!strcmp(arg_vet[i], "-sim"))
			{
				...
				NoximGlobalParams::mode_target_packets = false;
			}
			else if (!strcmp(arg_vet[i], "-packets"))
			{
				int tmp_target_packets = atoi(arg_vet[++i]);
				//cout << "#[D] target packets: " << tmp_target_packets << endl;
				if(tmp_target_packets > 0)
				{
					NoximGlobalParams::target_packets = tmp_target_packets;
					NoximGlobalParams::simulation_time = MAX_SIMULATION_TIME;
					NoximGlobalParams::mode_target_packets = true;
				}
				else
				{
					NoximGlobalParams::simulation_time = DEFAULT_SIMULATION_TIME;
					NoximGlobalParams::mode_target_packets = false;
				}
			}
			...
			else if (!strcmp(arg_vet[i], "-frouters")) 
			{
				while(++i < arg_num && arg_vet[i][0] != '-')
				{
					NoximGlobalParams::faulty_routers.push_back(std::stoi(arg_vet[i]));
				}
				if(i < arg_num) --i;
			}
			else
