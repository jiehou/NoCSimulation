							  ...
							  const NoximFlit &flit)
{
	...
	//@DEBUG
	//debugFlit(flit, arrival_time);
	/*
	map<string, vector<int>>::iterator it = NoximGlobalParams::hops_info.find(flitToString(flit));
	if(it != NoximGlobalParams::hops_info.end())
	{
		if(flit.flit_type == FLIT_TYPE_HEAD)
		{
			cout << "#[D] src: " << flit.src_id << ", dst: " << flit.dst_id << ", generate: " << flit.timestamp 
				<< ", arrive: " << arrival_time  << endl;
			cout << "	";
			for(auto router : it->second)
			{
				cout << router << " ";
			}
			cout << endl;
		}
		//NoximGlobalParams::hops_info.erase(it);
	}
	*/
