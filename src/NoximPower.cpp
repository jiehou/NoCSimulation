...
NoximPower::NoximPower()
{
  ...
  if (!power_data_loaded)
    {
			string file_path = string(NOXIM_DIR) + string(NoximGlobalParams::router_power_filename);
      assert(LoadPowerData(file_path));
...
bool NoximPower::LoadPowerData(const string& fname)
{
