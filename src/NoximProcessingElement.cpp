...
void NoximProcessingElement::txProcess()
{
	...
	else
	{
		...
		// if it is faulty, do nothing
		if(!isRouterGood(local_id))
			return;
		
		bool should_push = true;
		// do not push into buffer if the generated packets exceed the target_packets
		if(NoximGlobalParams::mode_target_packets && (sent_total == static_cast<unsigned>(NoximGlobalParams::target_packets)))
		{
			should_push = false;
		}

		NoximPacket packet;
		if (canShot(packet) && should_push)
		{
			double current_cycle = sc_time_stamp().to_double()/1000;
			transmittedAtPreviousCycle = true;
			sent_total++;
			if(NoximGlobalParams::verbose_mode > VERBOSE_OFF)
				cout << "#[D] PE "<< local_id << ", cycle:" << current_cycle << ", sent_total: " << sent_total << endl;
		...
		if (ack_tx.read() == current_level_tx)
		{
			if (!packet_queue.empty())
			{
				...
				if (NoximGlobalParams::verbose_mode > VERBOSE_OFF)
				{
					cout << "#[D] PE: " << sc_time_stamp().to_double() / 1000
						 << ": ProcessingElement[" << local_id << "] SENDING " << flit << endl;
				}
...
bool NoximProcessingElement::canShot(NoximPacket &packet)
{
	...
	// check packet's destination router status
	/*
	if(!isRouterGood(packet.dst_id))
		shot = false;
	*/
	return shot;
}
...
NoximPacket NoximProcessingElement::trafficShuffle()
{
	...
	p.src_id = local_id;
	p.dst_id = dnode;
	if(dnode >= NoximGlobalParams::mesh_dim_x * NoximGlobalParams::mesh_dim_y)
	{
		cout << "#[D] nbits: " << nbits << ", dnode: " << dnode << endl;
	}
	p.timestamp = sc_time_stamp().to_double() / 1000;
	p.size = p.flit_left = getRandomSize();
